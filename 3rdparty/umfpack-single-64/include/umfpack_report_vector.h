...
(
    int n,
    const UMFPACK_MYFLOAT Xx [ ], const UMFPACK_MYFLOAT Xz [ ],
    const UMFPACK_MYFLOAT Control [UMFPACK_CONTROL]
) ;
...
UMFPACK_MYFLOAT int Syntax:
    ...
    #include "umfpack.h"
    int n, status ;
    UMFPACK_MYFLOAT *X, Control [UMFPACK_CONTROL] ;
...
complex int Syntax:
    ...
    #include "umfpack.h"
    int n, status ;
    UMFPACK_MYFLOAT *Xx, *Xz, Control [UMFPACK_CONTROL] ;
...
Arguments:
    ...
    UMFPACK_MYFLOAT X [n] ;	Input argument, not modified.  For real versions.

	A real vector of size n.  X must not be (UMFPACK_MYFLOAT *) NULL.

    UMFPACK_MYFLOAT Xx [n or 2*n] ; Input argument, not modified.  For complex versions.
    UMFPACK_MYFLOAT Xz [n or 0] ;   Input argument, not modified.  For complex versions.

	A complex vector of size n, in one of two storage formats.
	Xx must not be (UMFPACK_MYFLOAT *) NULL.

	If Xz is not (UMFPACK_MYFLOAT *) NULL, then Xx [i] is the real part of X (i) and
	Xz [i] is the imaginary part of X (i).  Both vectors are of length n.
	This is the "split" form of the complex vector X.

	If Xz is (UMFPACK_MYFLOAT *) NULL, then Xx holds both real and imaginary parts,
	...
	compiler with the intrinsic UMFPACK_MYFLOAT _Complex type, then Xx can be of
	type UMFPACK_MYFLOAT _Complex in the calling routine and typecast to (UMFPACK_MYFLOAT *)
	...
	The packed format is compatible with the intrinsic UMFPACK_MYFLOAT _Complex
