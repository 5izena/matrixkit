...
UMFPACK_MYFLOAT int Syntax:
    ...
    int *Ap, *Ai, status ;
    UMFPACK_MYFLOAT *Ax, Control [UMFPACK_CONTROL], Info [UMFPACK_INFO] ;
...
complex int Syntax:
    ...
    void *Symbolic, *Numeric ;
    int *Ap, *Ai, status ;
    UMFPACK_MYFLOAT *Ax, *Az, Control [UMFPACK_CONTROL], Info [UMFPACK_INFO] ;
...
Arguments:
    ...
    UMFPACK_MYFLOAT Ax [nz] ;	Input argument, not modified, of size nz = Ap [n_col].
    ...
    UMFPACK_MYFLOAT Az [nz] ;	Input argument, not modified, for complex versions.
	...
	**Numeric is the address of a (void *) pointer variable in the user's
	...
	Control [UMFPACK_ALLOC_INIT]:
	    ...
	    object.  Part of this is of fixed size (approximately n UMFPACK_MYFLOAT's +
	...
	(UMFPACK_MYFLOAT *) NULL pointer is passed, then no statistics are returned in
