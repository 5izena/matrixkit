...
(
    ...
    const int Ti [ ],
    const int Tj [ ],
    const UMFPACK_MYFLOAT Tx [ ], const UMFPACK_MYFLOAT Tz [ ],
    int Ap [ ],
    int Ai [ ],
    UMFPACK_MYFLOAT Ax [ ], UMFPACK_MYFLOAT Az [ ],
    int Map [ ]
) ;
...
Arguments:
    ...
    UMFPACK_MYFLOAT Tx [nz] ;	Input argument, not modified.
			Size 2*nz if Tz or Az are NULL.
    UMFPACK_MYFLOAT Tz [nz] ;	Input argument, not modified, for complex versions.
	...
	(not (UMFPACK_MYFLOAT *) NULL).  This is not error condition; the routine can
    ...
    UMFPACK_MYFLOAT Ax [nz] ;	Output argument.  Size 2*nz if Tz or Az are NULL.
    UMFPACK_MYFLOAT Az [nz] ;	Output argument for complex versions.

	Ax and Az (for the complex versions) are UMFPACK_MYFLOAT arrays of size nz on
	input.  Note that only the first Ap [n_col] entries are used
	in both arrays.

	Ax is optional; if Tx and/or Ax are not present (a (UMFPACK_MYFLOAT *) NULL
