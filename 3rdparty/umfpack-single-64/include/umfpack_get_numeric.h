...
(
    int Lp [ ],
    int Lj [ ],
    UMFPACK_MYFLOAT Lx [ ], UMFPACK_MYFLOAT Lz [ ],
    int Up [ ],
    int Ui [ ],
    UMFPACK_MYFLOAT Ux [ ], UMFPACK_MYFLOAT Uz [ ],
    int P [ ],
    int Q [ ],
    UMFPACK_MYFLOAT Dx [ ], UMFPACK_MYFLOAT Dz [ ],
    int *do_recip,
    UMFPACK_MYFLOAT Rs [ ],
    void *Numeric
) ;
...
complex int Syntax:
    ...
    void *Numeric ;
    int *Lp, *Lj, *Up, *Ui, *P, *Q, status, do_recip ;
    UMFPACK_MYFLOAT *Lx, *Lz, *Ux, *Uz, *Dx, *Dz, *Rs ;
...
Purpose:
    ...
    #define noD (UMFPACK_MYFLOAT *) NULL
    #define noI (int *) NULL
...
Arguments:
    ...
    UMFPACK_MYFLOAT Lx [lnz] ;	Output argument.  Size 2*lnz for packed complex case.
    UMFPACK_MYFLOAT Lz [lnz] ;	Output argument for complex versions.
    ...
    UMFPACK_MYFLOAT Ux [unz] ;	Output argument. Size 2*unz for packed complex case.
    UMFPACK_MYFLOAT Uz [unz] ;	Output argument for complex versions.
    ...
    UMFPACK_MYFLOAT Dx [min(n_row,n_col)] ;	Output argument.  Size 2*n for
					the packed complex case.
    UMFPACK_MYFLOAT Dz [min(n_row,n_col)] ;	Output argument for complex versions.
    ...
    UMFPACK_MYFLOAT Rs [n_row] ;		Output argument.
